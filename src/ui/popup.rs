...
fn player_input(
    ...
    if let Some(key) = key {
        match key {
            ...
            KeyCode::Return => {
                // activate selected item and close inventory window
...
fn despawn_menu(mut commands: Commands, query_inventory: Query<Entity, With<InventoryUI>>) {
...
impl Plugin for PopUpPlugin {
    fn build(&self, app: &mut App) {
        app.add_event::<ChosenItemEvent>()
            ...
            .add_systems(
                Update,
                player_input.run_if(in_state(PopUpState::InventoryPopup)),
            )
            .add_systems(
                Update,
                player_input.run_if(in_state(PopUpState::EquipmentPopup)),
            )
